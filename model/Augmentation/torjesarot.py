...
m = 96000
cm =open('A321rot_edit1.txt','r')
cm = cm.read()
cm = cm.split()
cm = np.array(cm).reshape((-1,3200))
cm = cm.astype(float)
jeff =open('jefflodrot.txt','r')
jeff = jeff.read()
jeff = jeff.split()
jeff = np.array(jeff)
...
x = (x - x_mean) / x_scale
np.savetxt('rotmean_edit1',x_mean, fmt='%s',delimiter=' ' )
np.savetxt('rotstd_edit1',x_scale, fmt='%s',delimiter=' ' )
print('finish')
...
class MultipleLayerRegressor(nn.Module):
   ...
   def forward(self, x):
       ...
       x = self.out(x)
       return x
mlp = MultipleLayerRegressor(n_feature=3200,n_hid1=6400,n_hid2=400,n_output=1)
