...
def decrmake1(i,j):
    ...
    jC1 = jlist[0:10]
    jC2 = jlist[15:25]
    iC = np.append(iC2,iC1).reshape(20,-1)
    jC = np.append(jC1,jC2).reshape(20,-1)
    cminter = np.empty([0,0])
    for a in range(0,20):
        for b in range(0,20):
...
def decrmake2(i,j):
    ...
    jC2 = jlist[15:25]
    iC = np.append(iC2,iC1).reshape(20,-1)
    jC = np.append(jC2,jC1).reshape(20,-1)
    cminter = np.empty([0,0])
    for a in range(0,20):
        for b in range(0,20):
            xij = iC[a,0]-jC[b,0]
...
for k in range(0,m):
    inp = np.append(inp,decrmake1(id[k,0]-1,id[k,1]-1))
for k in range(0,m):
    inp = np.append(inp,decrmake2(id[k,0]-1,id[k,1]-1))
for k in range(0,m):
    inp = np.append(inp,decrmake3(id[k,0]-1,id[k,1]-1))
inp = inp.reshape(-1,3600)
np.savetxt('A321exx0rot.txt',inp, fmt='%s',delimiter=' ' )
