...
class MultipleLayerRegressor(nn.Module):
   ...
   def forward(self, x):
       ...
       x = self.out(x)
       return x
mlp = MultipleLayerRegressor(n_feature=3600,n_hid1=7200,n_hid2=484,n_output=1)
loss_fn = torch.nn.L1Loss(reduction='mean')
learning_rate = 1e-5
optimizer = torch.optim.Adam(mlp.parameters(), lr=learning_rate, weight_decay=2E-5) 
bat_siz = 600
