...
m = 32181
cm =open('A3210h1_edit.txt','r')
cm = cm.read()
cm = cm.split()
cm = np.array(cm).reshape((-1,3200))
cm = cm.astype(float)
jeff =open('jefflod0h1.txt','r')
jeff = jeff.read()
jeff = jeff.split()
jeff = np.array(jeff)
...
np.savetxt('0h1mean',x_mean, fmt='%s',delimiter=' ' )
np.savetxt('0h1std',x_scale, fmt='%s',delimiter=' ' )
...
learning_rate = 1e-7
optimizer = torch.optim.Adam(mlp.parameters(), lr=learning_rate, weight_decay=5E-6) 
bat_siz = 600
